...
/*
    ...
    根据地址在当前句柄中读取位大小
*/
static mdSTATUS mdReadBit(RegisterPoolHandle handler, mdU32 addr,mdBit *abit) reentrant {
    mdSTATUS ret = mdFALSE;
    ...
    if (ret == mdTRUE)
    {
        (*abit) = ToBit( mdGetBit(reg->dat,reg_off));
    }
    else{
        (*abit) = mdLow;
    }
...
/*
    ...
    根据地址修改当前句柄中的位大小
*/
static mdSTATUS mdWriteBit(RegisterPoolHandle handler,mdU32 addr,mdBit abit) reentrant{
    mdSTATUS ret = mdFALSE;
    mdU32 reg_addr = mdREG_ADDR(addr);
    mdU32 reg_off = mdREG_OFFSET(addr);
    RegisterHandle reg;
...
*/
static mdSTATUS mdReadBits(RegisterPoolHandle handler,mdU32 addr,mdU32 len,mdBit *bits) reentrant{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
*/
static mdSTATUS mdWriteBits(RegisterPoolHandle handler,mdU32 addr,mdU32 len,mdBit* bits) reentrant{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
/*
    ...
    根据地址读取一个寄存器值
*/
static mdSTATUS mdReadU16(RegisterPoolHandle handler,mdU32 addr,mdU16 *dat) reentrant{
    mdSTATUS ret = mdFALSE;
    RegisterHandle reg;
...
/*
    ...
    根据地址写入一个寄存器
*/
static mdSTATUS mdWriteU16(RegisterPoolHandle handler,mdU32 addr,mdU16 dat) reentrant{
    mdSTATUS ret = mdFALSE;
    RegisterHandle reg;
    ret = mdFindRegisterByAddress(handler,addr,&reg);
    if (ret == mdTRUE)
...
/*
    ...
    根据地址读取一组寄存器值
*/
static mdSTATUS mdReadU16s(RegisterPoolHandle handler,mdU32 addr,mdU32 len,mdU16 *dat) reentrant{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
	
    for (i = 0; i < len; i++)
    {
        ret = handler->mdReadU16(handler,addr+i, dat++);
    }
    return ret;
}
...
/*
    ...
    根据地址写入一组寄存器值
*/
static mdSTATUS mdWriteU16s(RegisterPoolHandle handler,mdU32 addr,mdU32 len,mdU16 *dat) reentrant{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
	
    for (i = 0; i < len; i++)
    {
        ret = handler->mdWriteU16(handler,addr+i,*(dat++));
    }
    return ret;
}
...
static mdSTATUS mdReadCoil(RegisterPoolHandle handler, mdU32 addr, mdBit* abit) reentrant
{
    mdU16 buf;
...
static mdSTATUS mdReadCoils(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdBit* bits) reentrant
{
    mdU16 buf;
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
static mdSTATUS mdWriteCoil(RegisterPoolHandle handler, mdU32 addr, mdBit abit) reentrant
{
    return handler->mdWriteU16(handler, addr + COIL_OFFSET, (mdU16)abit);
}

static mdSTATUS mdWriteCoils(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdBit* bits) reentrant
{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
static mdSTATUS mdReadInputCoil(RegisterPoolHandle handler, mdU32 addr, mdBit* abit) reentrant
{
    mdU16 buf;
...
static mdSTATUS mdReadInputCoils(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdBit* bits) reentrant
{
    mdU16 buf;
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
static mdSTATUS mdWriteInputCoil(RegisterPoolHandle handler, mdU32 addr, mdBit abit) reentrant
{
    return handler->mdWriteU16(handler, addr + INPUT_COIL_OFFSET, (mdU16)abit);
}

static mdSTATUS mdWriteInputCoils(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdBit* bits) reentrant
{
    mdSTATUS ret = mdFALSE;
	mdU32 i = 0;
...
static mdSTATUS mdReadInputRegister(RegisterPoolHandle handler, mdU32 addr, mdU16* dat) reentrant
{
    return handler->mdReadU16(handler, addr + INPUT_REGISTER_OFFSET, dat);
}

static mdSTATUS mdReadInputRegisters(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdU16* dat) reentrant
{
    return handler->mdReadU16s(handler, addr + INPUT_REGISTER_OFFSET, len, dat);
}

static mdSTATUS mdWriteInputRegister(RegisterPoolHandle handler, mdU32 addr, mdU16 dat) reentrant
{
    return handler->mdWriteU16(handler, addr + INPUT_REGISTER_OFFSET, dat);
}

static mdSTATUS mdWriteInputRegisters(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdU16* dat) reentrant
{
    return handler->mdWriteU16s(handler, addr + INPUT_REGISTER_OFFSET, len, dat);
}


static mdSTATUS mdReadHoldRegister(RegisterPoolHandle handler, mdU32 addr, mdU16* dat) reentrant
{
    return handler->mdReadU16(handler, addr + HOLD_REGISTER_OFFSET, dat);
}

static mdSTATUS mdReadHoldRegisters(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdU16* dat) reentrant
{
    return handler->mdReadU16s(handler, addr + HOLD_REGISTER_OFFSET, len, dat);
}

static mdSTATUS mdWriteHoldRegister(RegisterPoolHandle handler, mdU32 addr, mdU16 dat) reentrant
{
    return handler->mdWriteU16(handler, addr + HOLD_REGISTER_OFFSET, dat);
}

static mdSTATUS mdWriteHoldRegisters(RegisterPoolHandle handler, mdU32 addr, mdU32 len, mdU16* dat) reentrant
{
    return handler->mdWriteU16s(handler, addr + HOLD_REGISTER_OFFSET, len, dat);
}
...
mdSTATUS mdCreateRegisterPool(RegisterPoolHandle* regpoolhandle){
    ...
    if (handler != NULL)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            for (j = 0; j < REGISTER_POOL_MAX_BUFFER; j++)
            {
                RegisterHandle reg;
                ret = mdCreateRegister(handler,&reg);
                if (ret == mdFALSE) goto exit;
